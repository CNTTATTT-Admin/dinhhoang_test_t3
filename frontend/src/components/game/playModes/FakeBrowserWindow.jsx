import { Building2, Link2, Lock, Shield, X } from 'lucide-react'
...
export default function FakeBrowserWindow({
  ...
  const normalizedWebType = String(webType || 'MICROSOFT').toUpperCase()
  const webConfigs = {
    FACEBOOK: { color: '#1877F2', title: 'Facebook Login', Logo: Building2, buttonLabel: 'Continue to Facebook' },
    GOOGLE: { color: '#4285F4', title: 'Sign in with Google', Logo: Building2, buttonLabel: 'Sign in with Google' },
    MICROSOFT: { color: '#00A4EF', title: 'Microsoft 365 Security', Logo: Shield, buttonLabel: 'Verify Microsoft Account' },
    GITHUB: { color: '#24292e', title: 'GitHub - Sign In', Logo: Building2, buttonLabel: 'Sign in to GitHub' },
    FINANCE: { color: '#059669', title: 'Hệ thống Kế toán Nội bộ', Logo: Lock, buttonLabel: 'Đăng nhập Finance' },
  }
  const cfg = webConfigs[normalizedWebType] || webConfigs.MICROSOFT
  const HeaderLogo = cfg.Logo || Shield

  return (
    <section
      className="flex h-full min-h-0 flex-col bg-[#dee1e6] text-slate-800"
    ...
    >
      ...
      <div className="window-header flex shrink-0 cursor-grab items-center gap-2 border-b border-slate-300/80 bg-[#e8eaed] px-2 py-2 active:cursor-grabbing">
        ...
        <div className="ml-1 flex min-w-0 flex-1 items-center gap-2 rounded-full border border-slate-300/90 bg-white px-3 py-1.5 font-mono text-[11px] text-slate-600 shadow-sm">
          <HeaderLogo className="h-3.5 w-3.5 shrink-0" style={{ color: cfg.color }} />
        ...
        <button
          ...
          onClick={onClose}
          className="rounded p-1 text-slate-600 hover:bg-slate-300 disabled:cursor-not-allowed disabled:opacity-40"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-gradient-to-b from-slate-200/90 to-slate-300/80 p-4 sm:p-8">
        <div className="mx-auto max-w-md rounded-xl border border-white/60 bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
          <div className="flex items-center gap-2">
            ...
            >
              <HeaderLogo className="h-4 w-4" />
            </span>
            <h2 className="text-lg font-bold text-slate-900">{title || cfg.title}</h2>
          </div>
          ...
          <button
            ...
            className="mt-6 w-full rounded-lg py-3 text-sm font-semibold text-white shadow-md transition disabled:cursor-not-allowed disabled:bg-slate-300"
            style={{ backgroundColor: cfg.color }}
          >
            {isOtp ? 'Xác nhận OTP' : cfg.buttonLabel || 'Đăng nhập'} — tiếp tục
          </button>
